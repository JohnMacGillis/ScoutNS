...
export const findMatchingRuleIndex = (water: SpecialWater, rules: FishingRule[]): number | null => {
  ...
  if (water.ruleId !== undefined) {
    const idMatch = rules.findIndex(rule => rule.id === water.ruleId);
    if (idMatch !== -1) {
  ...
  if (water.location) {
    const locationIndex = rules.findIndex(rule => 
      rule.area && 
      water.location && 
      normalizeText(rule.area).includes(normalizeText(water.location))
    );
...
export const findMatchingSpecialWater = (rule: FishingRule, specialWaters?: SpecialWater[] | Record<string, SpecialWater[]>): SpecialWater | null => {
  ...
  try {
    if (rule.id !== undefined) {
      const idMatch = watersList.find(water => water && water.ruleId === rule.id);
      if (idMatch) {
  ...
  try {
    const locationMatch = watersList.find(water => 
      water && water.location && rule.area && 
      normalizeText(water.location) === normalizeText(rule.area)
    );
